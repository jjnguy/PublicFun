...
public class ClockPanel extends JPanel {
	...
	private void drawGenericHand(double angle, double length, double angleDif,
			double wideLength, boolean border, Graphics2D g) {
		Point o = getOrigin();
	...
	private void paintTicks(Graphics2D g) {
		for (int i = 0; i < 60; i++) {
			...
			if (i % 15 == 0) {
				...
				g
						.drawLine(moreInner.x, moreInner.y, innerPoint.x,
								innerPoint.y);
				g.setStroke(new BasicStroke(1));
			}
			if (i % 5 == 0) {
				g.setStroke(new BasicStroke(2));
				g.drawLine(outerPoint.x, outerPoint.y, innerPoint.x,
						innerPoint.y);
				g.setStroke(new BasicStroke(1));
			}
		}
